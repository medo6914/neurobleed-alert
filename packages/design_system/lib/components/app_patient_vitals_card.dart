    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GridView.count(
          ...
          children: [
            _buildVitalTile(
              'HR', '${vitals.heartRate.toInt()}', 'bpm',
              NeuroColors.heartRate, theme,
            ),
            _buildVitalTile(
              'SpO2', '${vitals.oxygenSaturation.toInt()}', '%',
              NeuroColors.oxygenSaturation, theme,
            ),
            _buildVitalTile(
              'BP', '${vitals.systolicBP.toInt()}/${vitals.diastolicBP.toInt()}', 'mmHg',
              NeuroColors.bloodPressureSystolic, theme,
            ),
            _buildVitalTile(
              'Temp', vitals.temperature.toStringAsFixed(1), '°C',
              NeuroColors.temperature, theme,
            ),
            _buildVitalTile(
              'RR', '${vitals.respiratoryRate.toInt()}', '/min',
              NeuroColors.respiratoryRate, theme,
            ),
            _buildVitalTile(
              'ICP', '${vitals.icp.toInt()}', 'mmHg',
              NeuroColors.icp, theme,
            ),
            _buildVitalTile(
              'CPP', '${vitals.cpp.toInt()}', 'mmHg',
              NeuroColors.cpp, theme,
            ),
            _buildVitalTile(
              'O2', '${vitals.oxygenSaturation.toInt()}', '%',
              NeuroColors.oxygenSaturation, theme,
            ),
          ],
        );
