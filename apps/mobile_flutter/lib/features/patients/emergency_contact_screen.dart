...
class _EmergencyContactScreenState
  ...
  Widget build(BuildContext context) {
    final contactsAsync =
        ref.watch(_emergencyContactsProvider(widget.patientId));
    final theme = Theme.of(context);

    return Scaffold(
      appBar:
          AppBar(title: const Text('Emergency Contacts'), centerTitle: true),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (contact.address != null)
                          _contactRow(Icons.location_on, contact.address!),
                      ],
                    ),
                  ),
                ),
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  await apiClient.post(
                      '/v1/patients/${widget.patientId}/emergency-contacts',
                      data: {
                        'name': nameCtrl.text,
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            style:
                ElevatedButton.styleFrom(backgroundColor: NeuroColors.critical),
            onPressed: () async {
              try {
                final apiClient = ref.read(apiClientProvider);
                await apiClient.delete(
                    '/v1/patients/${widget.patientId}/emergency-contacts/${contact.id}');
                Navigator.pop(context);
                ref.invalidate(_emergencyContactsProvider(widget.patientId));
              } catch (e) {
                ScaffoldMessenger.of(context)
            ...
            child: const Text('Delete',
                style: TextStyle(color: NeuroColors.textPrimary)),
          ),
        ],
      ),
    );
