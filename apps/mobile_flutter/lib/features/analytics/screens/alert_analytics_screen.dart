          ...
          child: Column(
            ...
            children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Average Response Time', style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
                          Text('${data.averageResponseTimeMinutes.toStringAsFixed(1)} minutes', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ],
                  ),
              ...
              if (data.bySeverity.isNotEmpty) ...[
                SizedBox(height: NeuroSpacing.lg),
                Text('By Severity', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                SizedBox(height: NeuroSpacing.sm),
                ...data.bySeverity.map((s) => Padding(
                  ...
                  child: AppCard(
                    child: Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(child: Text('${s['severity'] ?? 'Unknown'}', style: theme.textTheme.bodyMedium)),
                          Text('${s['count'] ?? 0}', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
              ...
              if (data.byType.isNotEmpty) ...[
                SizedBox(height: NeuroSpacing.lg),
                Text('By Type', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                SizedBox(height: NeuroSpacing.sm),
                ...data.byType.map((t) => Padding(
                  ...
                  child: AppCard(
                    child: Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(child: Text('${t['type'] ?? 'Unknown'}', style: theme.textTheme.bodyMedium)),
                          Text('${t['count'] ?? 0}', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
...
class _AlertSeverityCard extends StatelessWidget {
  ...
  const _AlertSeverityCard({required this.label, required this.value, required this.color});
  ...
  Widget build(BuildContext context) {
    ...
    return AppCard(
      child: Padding(
        padding: EdgeInsets.symmetric(vertical: NeuroSpacing.sm, horizontal: NeuroSpacing.xs),
        child: Column(
          children: [
