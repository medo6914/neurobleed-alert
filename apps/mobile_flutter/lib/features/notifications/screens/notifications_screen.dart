...
final alertFeedProvider =
    FutureProvider<List<Map<String, dynamic>>>((ref) async {
  final api = ref.read(apiClientProvider);
  final response =
      await api.get('/v1/alerts/', queryParameters: {'per_page': 15});
  final data = response.data;
  if (data is List) return data.cast<Map<String, dynamic>>();
  if (data is Map && data['items'] is List)
    return (data['items'] as List).cast<Map<String, dynamic>>();
        ...
        child: subscriptionsAsync.when(
          ...
          data: (subscriptions) {
            ...
            final filtered = subscriptions
                .where((s) => !_dismissedIds.contains(subscriptions.indexOf(s)))
                .toList();
            ...
            return ListView(
              ...
              children: [
                ...
                if (activeSubs.isNotEmpty) ...[
                  ...
                  ...activeSubs
                      .map((sub) => _buildSubscriptionCard(sub, theme)),
                  const SizedBox(height: 16),
                ],
      ...
      return [
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.notifications_none,
                  color: NeuroColors.textSecondary, size: 18),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (endDate.isNotEmpty)
                    Text('Ends: $endDate', style: theme.textTheme.bodySmall),
                  Row(children: [
                    Icon(Icons.check_circle,
                        size: 14, color: NeuroColors.success),
                    const SizedBox(width: 4),
                    Text(autoRenew ? 'Auto-renew' : 'Manual renew',
                        style: const TextStyle(fontSize: 11)),
                  ]),
                ],
              ),
...
class _AlertEventTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final type =
        (alert['alert_type'] as String? ?? 'general').replaceAll('_', ' ');
              ...
              children: [
                Row(
                  children: [
                    ...
                    if (acknowledged)
                      const Text('✓',
                          style: TextStyle(
                              color: NeuroColors.success, fontSize: 12)),
                  ],
                ),
                ...
                if (createdAt.isNotEmpty)
                  Text(_formatTime(createdAt),
                      style: theme.textTheme.bodySmall?.copyWith(
                          color: NeuroColors.textSecondary, fontSize: 11)),
              ],
            ),
          ),
        ],
