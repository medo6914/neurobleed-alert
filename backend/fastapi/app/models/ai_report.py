...
class AIReport(TimestampMixin, SoftDeleteMixin, VersionMixin, AuditMixin, FHIRMixin, MedicalCodeMixin, Base):
    ...
    report_type: Mapped[ReportType] = mapped_column(SAEnum(ReportType), nullable=False, default=ReportType.RISK_ASSESSMENT)
    ...
    herniation_risk: Mapped[HerniationRisk | None] = mapped_column(SAEnum(HerniationRisk), nullable=True)
    ...
    is_reviewed: Mapped[bool] = mapped_column(Boolean, default=False, server_default="0")

    patient = relationship("Patient", back_populates="ai_reports")


Index("ix_ai_reports_patient_type", AIReport.patient_id, AIReport.report_type, AIReport.created_at.desc())
