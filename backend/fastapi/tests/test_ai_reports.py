...
pytest.skip("Requires FastAPI client fixture + AI endpoints", allow_module_level=True)
...
async def test_ai_report_list(client):
    ...
    response = await client.get(
        "/v1/ai/reports", headers={"Authorization": f"Bearer {token}"}
    )
...
async def test_ai_report_detail(client):
    ...
    await client.post(
        f"/v1/ai/risk-assess/{patient_id}", headers={"Authorization": f"Bearer {token}"}
    )
    ...
    if reports.json():
        ...
        detail = await client.get(
            f"/v1/ai/reports/{report_id}", headers={"Authorization": f"Bearer {token}"}
        )
